...
//   // Масив містить лише структуру. Плейсхолдери генеруються автоматично в JSX.
...
//     try {
//       const response = console.log(bookingData)(bookingData);
...
//                 // Плейсхолдер формується динамічно: name_placeholder, email_placeholder тощо.
...
//               // Тепер плейсхолдер береться з локалей
...
const FormBooking = () => {
  ...
  const onSubmit = async currentFormData => {
    // 1. Отримуємо перекладену назву зони
    const translatedZone = currentFormData.selectedZone
      ? t(`atmosphere.zones.${currentFormData.selectedZone}`)
      : t('atmosphere.not_selected');

    // 2. Формуємо об'єкт для відправки
    const bookingData = {
      camperId: '64f1a2b3c4d5e6f7a8b9c0d1',
      name: currentFormData.name.trim(),
      email: currentFormData.email.trim(),
      phone: currentFormData.phone.trim(),
      selectedZone: translatedZone, // Тепер тут буде назва
      comment: currentFormData.comment.trim(),
      bookingStartDate:
        currentFormData.bookingStartDate?.toLocaleString() || null,
    };
    ...
    try {
      console.log('Дані, що відправляються на сервер:', bookingData);
  ...
  // ВИПРАВЛЕНО: Функція вибору зони тепер імітує подію input,
  // щоб хук useForm правильно зафіксував зміну значення.
  const handleZoneSelect = useCallback(
    zoneId => {
      handleInputChange({
        target: {
