...
//             className={css['success-button']} // Додаємо цей клас
//           >
//             {t('back_button', 'Close')}
//           </Button>
//         </div>
...
const ZONES = [
  { id: 'window', label: 'Біля вікна', img: '/images/window-interior.webp' },
  { id: 'lounge', label: 'Лаунж', img: '/images/lounge-interior.webp' },
  { id: 'work', label: 'Work-zone', img: '/images/work-interior.webp' },
];
...
const FormBooking = () => {
  ...
  const validationRules = useMemo(
    () => ({
      ...
      bookingStartDate: v => validateRequired(v, t),
      comment: v => validateComment(v, t, true),
      selectedZone: () => null, // Прибрано невикористаний аргумент 'v'
    }),
    [t]
  );
  ...
  const onSubmit = async formData => {
    const bookingData = {
      ...
      email: formData.email.trim(),
      phone: formData.phone.trim(),
      comment: `[Зона: ${formData.selectedZone}] ${formData.comment.trim()}`,
      bookingStartDate: formData.bookingStartDate?.toISOString() || null,
    };
  ...
  const toggleZone = zoneId => {
    handleInputChange({ target: { name: 'selectedZone', value: zoneId } });
  };
  ...
  return (
    <form className={css['form']} onSubmit={handleSubmit} noValidate>
      <div className={css['title-text-form']}>
        ...
        <div className={css['zone-selection-wrapper']}>
          <p className={css['zone-title']}>{t('choose_atmosphere')}</p>
          <div className={css['zone-scroll']}>
            {ZONES.map(zone => (
              <div
                key={zone.id}
                role="button"
                tabIndex={0}
                className={clsx(css['zone-card'], {
                  [css['zone-card--active']]: formData.selectedZone === zone.id,
                })}
                onClick={() => toggleZone(zone.id)}
                onKeyDown={e => e.key === 'Enter' && toggleZone(zone.id)}
              >
                <div className={css['zone-img-wrapper']}>
                  <img
                    src={zone.img}
                    alt={zone.label}
                ...
                <span className={css['zone-label']}>{zone.label}</span>
              </div>
            ))}
          </div>
        </div>
