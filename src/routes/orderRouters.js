...
// GET: Аналітика (ВИПРАВЛЕНО ТАЙМЗОНИ ДЛЯ НІЧНИХ ЗМІН)
router.get('/stats', async (req, res) => {
  try {
    const { date } = req.query;
    if (!date) return res.status(400).json({ message: 'Дата не вказана' });

    // Створюємо початок доби у локальному часі
    const startOfDay = new Date(date);
    startOfDay.setHours(0, 0, 0, 0);

    // Створюємо кінець доби у локальному часі
    const endOfDay = new Date(date);
    endOfDay.setHours(23, 59, 59, 999);

    console.log(`📊 Запит аналітики за дату: ${date}`);
    console.log(
      `🔍 Пошук в діапазоні ISO: від ${startOfDay.toISOString()} до ${endOfDay.toISOString()}`
    );
    ...
    const stats = await Order.aggregate([
      ...
      { $sort: { totalQuantity: -1 } }, // Популярні товари зверху
    ]);
